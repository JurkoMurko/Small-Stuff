...
const baseURL = 'dentist.csv'; const delayTime = 1000; 
...
async function processCsv() {
    ...
    const allRows = []; for await (const row of readStream) { allRows.push(row); }
    for (let i = 0; i < allRows.length; i += batchSize) {
        const batch = allRows.slice(i, i + batchSize); 
        const batchPromises = batch.map(async row => {
            const newRow = { ...row }; 
            const website = row.Website || ''; 
            if (website) { console.log(`Scraping: ${website}`); 
            const emails = await scrapeEmails(website); 
            emails.forEach((email, index) => { const emailHeader = `Email${index + 1}`; 
        ...
        batchResults.forEach(result => { if (result.status === 'fulfilled') { rows.push(result.value); } });
    }
    const headers = Object.keys(rows[0]).concat(Array.from(headersSet));
    return { rows, headers };
}
async function writeCsv({ rows, headers }) { const outputFileName = baseURL.replace('.csv', '_emails.csv'); 
const csvWriter = createCsvWriter({ path: outputFileName, header: headers.map(header => ({ id: header, title: header })) }); 
