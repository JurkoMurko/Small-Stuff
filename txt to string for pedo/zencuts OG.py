...
path4 = "C:\\Users\\quint\\OneDrive\\Desktop\\handwriting\\letters\\cropped\\paper"
...
def paster(w):
    ...
    for i in range (0,len(w)):
        ...
        if w[i] == -1:
            ...
            im2 = charList[w[i] -1]
            if line == 1:
                im2 = im2.resize((int(np.round(im2.width * random.uniform(2.90, 3.10))), int(np.round(im2.height * random.uniform(2.90, 3.10)))), Image.Resampling.NEAREST)
            else:
                im2 = im2.resize((int(np.round(im2.width * random.uniform(.90, 1.10))), int(np.round(im2.height * random.uniform(.90, 1.10)))), Image.Resampling.NEAREST)
            
            if  w[i] in mDTrans:
                print("\n\ndown")
                back_im.paste(im2, (625 + xtraD, 1271 + int(np.round(im2.height * random.uniform(1.10, 1.40) + xtraH))))
                ...
                print("else")
                back_im.paste(im2, (625 + xtraD, 1271 - xtraH))
